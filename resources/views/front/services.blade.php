                          ...
                          <div class="col-lg-5 col-md-5 col-sm-12 col-12">
                              <div class="row">
                                  <div class="col-lg-6 col-md-6 col-sm-12 col-12">
                                      <div class="img_box">
                                          <img src="{{ asset('service-profile/' . $service->ser_profile) }}" />
                                      </div>
                                  </div>
                                  <div class="col-lg-6 col-md-6 col-sm-12 col-12">
                                      <div class="info_text_box_first pt-4">
                                          <h1 class="heading44px">{{ $service['full_name'] }}</h1>
                                          <p class="heading22px">{{ $service->business_name }}</p>
                                          <p>{{ $service['per_bio'] }}</p>
                              ...
                              <div class="row">
                                  <div class="col-lg-6 col-md-6 col-sm-12 col-12">
                                      <div class="middle_txt_box">
                                          ...
                                          {{-- <p>
                                          Lorem ipsum dolor sit amet, consectetur adipisicing elit. Numquam vero expedita fugiat illo quasi doloremque, in unde omnis sint assumenda! Quaerat in,
                                          reprehenderit corporis voluptatum natus sequi
                                  ...
                                  <div class="col-lg-6 col-md-6 col-sm-12 col-12">
                                      <div class="right_btn_box">
                                          <a href="{{ url('service_details/' . encrypt($service->id)) }}" class="service_btn">Quick Contact</a>
                                      </div>
                                  </div>
                              </div>
                          </div>
...
{{-- <script>
   ...
   // 🟢 Notification helper (auto-create if missing)
   if (!notification) {
   ...
   // 🟢 Create tag with label, key, and optional source
   function createTag(label, value, key, sourceElement = null) {
       value = value.trim();
       ...
       if (typeof showNotification === 'function') {
       showNotification(`Added: ${label}: ${value}`);
       }
   }
   ...
   const selectFields = [
       'health', 'holistic', 'leasing', 'transport', 'grooming',
       'recreational', 'performance', 'property', 'boarding',
       'farrier', 'consulting', 'retail', 'promotion'
   ];
   ...
   selectFields.forEach(fieldName => {
       ...
       select.addEventListener("change", () => {
           ...
           if (value && text && text !== "Select a service") {
           const label = fieldLabels[fieldName] || fieldName.charAt(0).toUpperCase() + fieldName.slice(1);
           createTag(label, text, fieldName, select);
           }
       });
       }
   });
   
   // 🟢 Handle text inputs: location, name
   ['location', 'name'].forEach(fieldName => {
       const input = form.querySelector(`[name="${fieldName}"]`);
       if (input) {
       const handler = () => {
           if (input.value.trim()) {
           const label = fieldName === 'location' ? 'Location' : 'Name';
           createTag(label, input.value.trim(), fieldName, input);
           }
       };
   ...
   // 🟢 RESTORE ALL TAGS ON PAGE LOAD (after form submit)
   document.addEventListener("DOMContentLoaded", () => {
       ...
       // Text fields
       if (getVal('location')) createTag("Location", getVal('location'), "location");
       if (getVal('name')) createTag("Name", getVal('name'), "name");
   
       // Select fields
       selectFields.forEach(fieldName => {
       const value = getVal(fieldName);
       if (value) {
           const select = form.querySelector(`select[name="${fieldName}"]`);
           const text = select?.querySelector(`option[value="${value}"]`)?.textContent?.trim() || value;
           const label = fieldLabels[fieldName] || fieldName.charAt(0).toUpperCase() + fieldName.slice(1);
           createTag(label, text, fieldName);
       }
       });
   });
</script> --}}
<script>
   ...
   // 🟢 Auto-create notification if missing
   if (!notification) {
     const notif = document.createElement("div");
     notif.id = "tagNotification";
     notif.style.cssText = `
   ...
   // 🟢 Create tag (works for both live and restored)
   function createTag(label, value, key) {
     value = value.trim();
     if (!value || value === "Select a service") return;
     ...
     tag.querySelector(".remove-tag").addEventListener("click", () => {
       ...
       // Clear the actual form field by name (key)
   ...
   // 🟢 Attach live listeners for selects
   selectFields.forEach(fieldName => {
     const select = form.querySelector(`select[name="${fieldName}"]`);
     if (select) {
       select.addEventListener("change", () => {
   ...
   // 🟢 Attach live listeners for text inputs
   ['location', 'name'].forEach(fieldName => {
     const input = form.querySelector(`[name="${fieldName}"]`);
     if (input) {
       const handler = () => {
         if (input.value.trim()) {
           ...
           createTag(label, input.value.trim(), fieldName);
         }
       };
       input.addEventListener("blur", handler);
       input.addEventListener("keydown", (e) => {
